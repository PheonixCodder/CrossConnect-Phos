...
import { OrdersRepositoryPort } from '../../../../domain/repositories/repository-ports';
...
export class OrdersRepository implements OrdersRepositoryPort {
  ...
  async getByExternalOrderIds(
    ...
    const uniqueExternalOrderIds = [...new Set(externalOrderIds)].filter(
      Boolean,
    );
    if (!uniqueExternalOrderIds.length) return [];

    const BATCH_SIZE = 500;
    const orders: Database['public']['Tables']['orders']['Row'][] = [];

    for (let i = 0; i < uniqueExternalOrderIds.length; i += BATCH_SIZE) {
      const batch = uniqueExternalOrderIds.slice(i, i + BATCH_SIZE);
