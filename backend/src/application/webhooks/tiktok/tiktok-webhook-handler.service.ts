...
import { mapTiktokOrderStatus } from '../../../infrastructure/external/connectors/tiktok/tiktok.mapper';

@Injectable()
export class TikTokWebhookHandlerService {
  private readonly logger = new Logger(TikTokWebhookHandlerService.name);
  ...
  async resolveStoreIdByShopId(tiktokShopId: string): Promise<string | null> {
  ...
  async processForStore(storeId: string, body: any) {
    switch (body.type) {
      case 1:
