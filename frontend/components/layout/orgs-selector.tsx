                ...
                <PopoverContent align="start" className="w-[220px] p-0">
                    <Command>
                        ...
                        <CommandList>
                            ...
                            {/* STORES BY PLATFORM */}
                            {Object.entries(storesByPlatform).map(([platform, platformStores]) => (
                                <CommandGroup key={platform}
                                              heading={PLATFORM_NAMES[platform as PlatformType] || "Other"}>
                                    {platformStores.map((s) => (
                                        <CommandItem key={s.id} onSelect={() => {
                                            setActiveStore(s);
                                            setIsStoreOpen(false);
                                        }}>
                                            <StoreIcon className="mr-2 h-4 w-4 opacity-70"/> <span
                                            className="truncate">{s.name}</span> {activeStore?.id === s.id && (
                                            <Check className="ml-auto h-4 w-4"/>)} </CommandItem>))} </CommandGroup>),)}
                        </CommandList>
                    </Command>
                </PopoverContent>
            </Popover>
